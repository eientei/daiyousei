...
func decodeAck(buf io.Reader, raw RawMessage) Message {
	return &AckMessage{
		Sequence: uint32(binutil.ReadInt(buf, 4)),
	}
}
func (msg *AckMessage) Encode(buf io.Writer) {
	binutil.WriteInt(buf, int(msg.Sequence), 4)
}
...
func decodeAmf(buf io.Reader, raw RawMessage) Message {
	return &AmfMessage{
		Data: raw.Data,
	}
}
func (msg *AmfMessage) Encode(buf io.Writer) {
	binutil.WriteBuf(buf, msg.Data)
}